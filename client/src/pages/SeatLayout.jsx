...
const SeatLayout = () => {
  ...
  const openRazorpayCheckout = async (paymentData, fallbackTitle = "Movie Ticket") => {
  ...
  const bookTickets = async () => {
    try {
      ...
      if (data.success) {
        await openRazorpayCheckout(data, show?.movie?.title);
      } else {
        toast.error(data.message);
      }
    } catch (error) {
        ...
        <div className="mt-5 space-y-1">
          {show.dateTime[date].map((item, index) => (
            <div
              ...
              className={`flex flex-col px-6 py-2 w-max rounded-r-md cursor-pointer transition ${
