        ...
        <table className="w-full border-collapse rounded-md overflow-hidden text-nowrap">
          ...
          <tbody className="text-sm font-light">
            {bookings?.map((item, index) => (
              ...
              >
                <td className="p-2 min-w-36 pl-5">
                  {String(item._id || "").slice(-8).toUpperCase() || "N/A"}
                </td>
                <td className="p-2 min-w-45 pl-5">
                  {item.isPaid
                    ? item.user?.name || item.user?.firstName || item.user || "N/A"
                    : "--"}
                </td>
                ...
                <td className="p-2">{item.isPaid ? "Paid" : "Pending"}</td>
              </tr>
            ))}
          </tbody>
        </table>
